...
def create_double_value_function(value_fn, Q_ensemble, *args, **kwargs):
    ...
    # as a Keras layer.
    value_fns = tuple(value_fn(*args, **kwargs) for i in range(Q_ensemble))
    return value_fns
...
VALUE_FUNCTIONS = {
    ...
    'double_feedforward_Q_function': lambda Q_ensemble, *args, **kwargs: (
        create_double_value_function(
            vanilla.create_feedforward_Q_function, Q_ensemble, *args, **kwargs)),
}
...
def get_Q_function_from_variant(variant, env, *args, **kwargs):
    ...
    Q_ensemble = Q_params['Q_ensemble']
    Q_elite = Q_params['Q_elite']
